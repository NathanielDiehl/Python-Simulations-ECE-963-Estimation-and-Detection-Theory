...
def draw_plots():   
    ...
    P01_text.set_text(r'$P_{0}(\Gamma_1)$  = '   + f"{P_y[0][1]:0.3f}")
    P10_text.set_text(r'$P_{1}(\Gamma_0)$  = '   + f"{P_y[1][0]:0.3f}")
    P11_text.set_text(r'$P_{1}(\Gamma_1)$  = '   + f"{P_y[1][1]:0.3f}")
...
# Sliders
mu0_slider_ax    = fig.add_axes([0.25, 0.30, 0.5, 0.03], facecolor=axis_color)
mu0_slider       = Slider(mu0_slider_ax, r'$\mu_0$', -1.0, 1.0, valinit=mu0_0)

mu1_slider_ax    = fig.add_axes([0.25, 0.25, 0.5, 0.03], facecolor=axis_color)
mu1_slider       = Slider(mu1_slider_ax, r'$\mu_1$', -1.0, 1.0, valinit=mu1_0)

sigma0_slider_ax = fig.add_axes([0.25, 0.20, 0.5, 0.03], facecolor=axis_color)
sigma0_slider    = Slider(sigma0_slider_ax, r'$\sigma_0$', 0.01, 1.0, valinit=sigma0_0)

sigma1_slider_ax = fig.add_axes([0.25, 0.15, 0.5, 0.03], facecolor=axis_color)
sigma1_slider    = Slider(sigma1_slider_ax, r'$\sigma_1$', 0.01, 1.0, valinit=sigma1_0)

tau_slider_ax    = fig.add_axes([0.25, 0.10, 0.5, 0.03], facecolor=axis_color)
tau_slider       = Slider(tau_slider_ax, r'$\tau^,$', -1.0, 1.0, valinit=tau_prime)

pi_slider_ax     = fig.add_axes([0.25, 0.05, 0.5, 0.03], facecolor=axis_color)
pi_slider        = Slider(pi_slider_ax, r'$\pi_0$', 0.01, 0.99, valinit=pi_0)


P00_text = fig.text(0.8, 0.85, r'$P_{0}(\Gamma_0)$  = ',   color='black', fontsize=10, visible=True)
P01_text = fig.text(0.8, 0.80, r'$P_{0}(\Gamma_1)$  = ',   color='black', fontsize=10, visible=True)
P10_text = fig.text(0.8, 0.75, r'$P_{1}(\Gamma_0)$  = ', color='black', fontsize=10, visible=True)
P11_text = fig.text(0.8, 0.70, r'$P_{1}(\Gamma_1)$  = ', color='black', fontsize=10, visible=True)
...
def sliders_on_changed(val):
    ...
    global pi_0;      pi_0       = pi_slider.val
...
pi_slider.on_changed(sliders_on_changed)
